...
1. **Select scope** — By type, category, or full library.

2. **Apply decision tree to each card** (see Conan-Skill Type Taxonomy + reference.md)
   ```
   Card: [Name]
   Step 1: WHY question? → Strategy/Principle/Standard
   Step 2: WHAT + Users interact?
     Apply product-specific decision tree from reference.md
...
4. **Check conformance** — For product-layer cards:
   ```
   Card: [Name]
   Governed domains touched:
   [Check each governed domain from reference.md → Conformance Obligations]
