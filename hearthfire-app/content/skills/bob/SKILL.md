...
- **Rationale** (`/rationale/`) — WHY we build and what constraints exist. Contains foundational frameworks, Strategies, Principles, and Standards.
- **Product** (`/product/`) — WHAT gets built. Contains product-specific types (see reference.md for this product's types), plus Systems, Agents, and Prompts.
...
For product-specific type folders, consult reference.md → Folder Structure.
...
**Trigger:** "Build [scope]" or "Create cards for [scope]"
...
Inventory arrives
      │
      ▼
Read source material for scope
      │
      ▼
Create cards (Job 1) ◄───────────┐
      │                          │
...
8. **Respect the two-layer split.** Strategies, Principles, and Standards go in `/rationale/`. Product-layer cards go in `/product/` (see reference.md for type-to-folder mapping).
